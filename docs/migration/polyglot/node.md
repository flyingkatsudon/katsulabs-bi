...
const boards = useNewApi
  ...
  : await fetch('/bdp/cboard/dashboard/getBoardList', { credentials: 'include' })
```
...
- 기존 CSS는 `src/main/webapp/cboard/` 에 유지
- React 로 이전 시 `frontend/public/cboard-assets/` 로 복사 후 `<link>` 또는 `@import`
