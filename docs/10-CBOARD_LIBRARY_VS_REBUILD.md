...
| 구분 | 레거시 | 리빌드 |
|------|--------|--------|
| **제품** | CBoard CE BI | InsightBoard (CBoard 호환) |
| **코드** | `org.cboard.*` 복사/포크 | `com.insightboard` + `dashboard` 패키지 **자가 구현** |
...
전체 작업이 끝나면 레거시 `bdp` WAR의 CBoard 부분은 **아카이브**하고, 운영은 `insight-board`만 유지하면 됩니다.
