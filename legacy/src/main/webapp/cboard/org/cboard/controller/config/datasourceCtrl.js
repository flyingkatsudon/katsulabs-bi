...
cBoard.controller('datasourceCtrl', function ($scope, $state, $stateParams, $http, ModalUtils, $uibModal, $filter, $q) {
    ...
    $http.get("/bdp/cboard/dashboard/getProviderList").success(function (response) {
    ...
    $scope.deleteDs = function (ds) {
        ...
        var resDs = [];
        var resWdg = [];
        var promiseDs = $http.get("/bdp/cboard/dashboard/getAllDatasetList").then(function (response) {
            if (!response) {
                return false;
            }
        ...
        var promiseWdg = $http.get("/bdp/cboard/dashboard/getAllWidgetList").then(function (response) {
            if (!response) {
                return false;
            }
        ...
        p.then(function () {
            ...
            ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-warning", "lg", function () {
                $http.post("/bdp/cboard/dashboard/deleteDatasource", {id: ds.id}).success(function (serviceStatus) {
    ...
    $scope.copyDs = function (ds) {
        ...
        $http.post("/bdp/cboard/dashboard/saveNewDatasource", {json: angular.toJson(data)}).success(function (serviceStatus) {
            if (serviceStatus.status == '1') {
                $scope.optFlag = 'none';
                getDatasourceList();
    ...
    $scope.changeDsView = function () {
        $scope.dsView = '/bdp/cboard/dashboard/getDatasourceView?type=' + $scope.curDatasource.type;
    };

    $scope.doDatasourceParams = function () {
        $http.get('/bdp/cboard/dashboard/getDatasourceParams?type=' + $scope.curDatasource.type).then(function (response) {
    ...
    $scope.saveNew = function () {
        ...
        $http.post("/bdp/cboard/dashboard/saveNewDatasource", {json: angular.toJson($scope.curDatasource)}).success(function (serviceStatus) {
            if (serviceStatus.status == '1') {
                $scope.optFlag = 'none';
                getDatasourceList();
                $scope.verify = {dsName:true,provider:true};
    ...
    $scope.saveEdit = function () {
        ...
        $http.post("/bdp/cboard/dashboard/updateDatasource", {json: angular.toJson($scope.curDatasource)}).success(function (serviceStatus) {
            if (serviceStatus.status == '1') {
                $scope.optFlag = 'none';
                getDatasourceList();
                $scope.verify = {dsName:true,provider:true};
    ...
    $scope.test = function () {
        ...
        $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $http.get('/bdp/cboard/dashboard/getConfigParams?type=' + $scope.datasource.type + '&page=test.html').then(function (response) {
                    $scope.params = response.data;
                    for(i in $scope.params){
                        var name = $scope.params[i].name;
                        var value = $scope.params[i].value;
                ...
                $scope.do = function () {
                    ...
                    $http.post("/bdp/cboard/dashboard/test", {
