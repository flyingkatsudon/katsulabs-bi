...
cBoard.controller('jobCtrl', function ($scope, $rootScope, $http, dataService, $uibModal, ModalUtils, $filter, $interval) {
    ...
    $scope.interval = $interval(function () {
        $http.get("/bdp/cboard/dashboard/getJobList").success(function (response) {
    ...
    $scope.runJob = function (job) {
        $http.post("/bdp/cboard/dashboard/execJob", {id: job.id}).success(function (serviceStatus) {
    ...
    $scope.editJob = function (job) {
        $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.ok = function () {
                    if (job) {
                        $http.post("/bdp/cboard/dashboard/updateJob", {json: angular.toJson($scope.job)}).success(function (serviceStatus) {
                            if (serviceStatus.status == '1') {
                        ...
                        $http.post("/bdp/cboard/dashboard/saveJob", {json: angular.toJson($scope.job)}).success(function (serviceStatus) {
                            if (serviceStatus.status == '1') {
                                ModalUtils.alert(translate("COMMON.SUCCESS"), "modal-success", "sm");
                                $scope.$parent.loadJobList();
                                $uibModalInstance.close();
    ...
    $scope.deleteJob = function (job) {
        ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-info", "lg", function () {
            $http.post("/bdp/cboard/dashboard/deleteJob", {id: job.id}).success(function (serviceStatus) {
                if (serviceStatus.status == '1') {
                    $scope.loadJobList();
                } else {
                    ModalUtils.alert(serviceStatus.msg, "modal-warning", "lg");
