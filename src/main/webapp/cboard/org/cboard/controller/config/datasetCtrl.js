...
cBoard.controller('datasetCtrl', function ($scope, $http, $state, $stateParams, dataService, $uibModal, ModalUtils, $filter, chartService, $timeout, uuid4) {
    ...
    var originalData = [];
    var updateUrl = "/bdp/cboard/dashboard/updateDataset";
    ...
    var getCategoryList = function () {
        $http.get("/bdp/cboard/dashboard/getDatasetCategoryList").success(function (response) {
    ...
    $scope.editDs = function (ds) {
        $http.post("/bdp/cboard/dashboard/checkDatasource", {id: ds.data.datasource}).success(function (response) {
    ...
    $scope.deleteDs = function (ds) {
        $http.get("/bdp/cboard/dashboard/getAllWidgetList").then(function (response) {
            if (!response) {
                return false;
            }
            var resDs = [];
            ...
            ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-warning", "lg", function () {
                $http.post("/bdp/cboard/dashboard/deleteDataset", {id: ds.id}).success(function (serviceStatus) {
    ...
    $scope.copyDs = function (ds) {
        ...
        $http.post("/bdp/cboard/dashboard/saveNewDataset", {json: angular.toJson(data)}).success(function (serviceStatus) {
            if (serviceStatus.status == '1') {
                $scope.optFlag = 'none';
                getDatasetList();
    ...
    $scope.save = function () {
        ...
        if ($scope.optFlag == 'new') {
            $http.post("/bdp/cboard/dashboard/saveNewDataset", {json: angular.toJson(ds)}).success(function (serviceStatus) {
                if (serviceStatus.status == '1') {
                    getCategoryList();
                    getDatasetList();
                    $scope.verify = {dsName: true};
