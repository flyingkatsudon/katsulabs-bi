...
cBoard.service('chartService', function ($q, dataService, chartPieService, chartLineService, chartFunnelService,
                                         ...
                                         chartRelationService, //FROM HERE, additional widgets
                                         chartWordBubbleService, chartGoogleMapService, chartFusionGanttchartsService, chartBoxplotService, 
                                         chartThemeRiverService, chartSunburstService, chartParallelService, chartPyramidService, chartParetoService) {
        ...
        this.render = function (containerDom, widget, optionFilter, scope, reload, persist, relations) {
            ...
            dataService.getDataSeries(widget.datasource, widget.query, widget.datasetId, widget.config, function (data) {
                try {
                    ...
                    } else if (widget.config.chart_type == 'wordBubble') {
                       deferred.resolve(chart.render(containerDom, option, scope, persist, data, relations, widget.config));
                    } else {
        ...
        var getChartServices = function (chartConfig) {
            ...
            switch (chartConfig.chart_type) {
            	...
            	case 'googleMap':
            		chart=chartGoogleMapService;
            		break;
            	case 'wordBubble':
                 	chart=chartWordBubbleService;
                 	break;
            	case 'fusionganttcharts':
 	                chart = chartFusionGanttchartsService;
 	                break;
            	case 'boxplot':
 	            	chart = chartBoxplotService;
 	            	break;
 	            case 'themeRiver':
 	            	chart = chartThemeRiverService;
 	            	break;
 	            case 'sunburst':
 	            ...
 	            case 'parallel':
 	            	...
 	            	break;
 	            case 'pyramid':
 	            	chart = chartPyramidService;
 	            	break;
 	            case 'pareto':
 	            	chart = chartParetoService;
 	            	break;      
            }
            return chart;
        };
