...
메인·서브 에이전트의 정책, 위임 규칙, Cursor 전용 설정:

- [docs/harness/README.md](docs/harness/README.md)
- [docs/harness/AGENT-HIERARCHY.md](docs/harness/AGENT-HIERARCHY.md)

**아키텍처 (backend):** `domain` ← `application` ← `api` 방향이며, `infrastructure` 가 포트 구현을 담당합니다. `domain` 에는 Spring 같은 프레임워크 의존을 두지 않습니다. JSON 은 `JsonMapper` 만 사용하고 Fastjson 은 사용하지 않습니다.

**Legacy / `dataJson`:** [docs/migration/04-legacy-layer-removal.md](docs/migration/04-legacy-layer-removal.md) 에 정의된 PR 순서를 따라 단계적으로 제거합니다.

**서브에이전트:** 서로 격리된 컨텍스트에서 동작하므로, 위임할 때마다 prompt 안에 “AGENTS.md 를 읽으라”는 지시와 구체적인 작업·완료 기준을 함께 적어야 합니다.
