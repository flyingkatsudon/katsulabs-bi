...
class SpaStaticIntegrationTest {
    ...
    void servesIndexWithoutAuth() throws Exception {
        ...
        assertThat(index.getResponse().getContentAsString()).contains("insight-board-spa-test");

        var boards = mockMvc.perform(get("/boards")).andExpect(status().isOk()).andReturn();
        assertThat(boards.getResponse().getContentAsString()).contains("insight-board-spa-test");
    }
